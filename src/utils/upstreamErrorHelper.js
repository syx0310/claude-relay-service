  ...
  try {
    ...
    accountChecks.forEach(({ accountId, accountType }, index) => {
      ...
      if (err || ttl === -2) {
        ...
        // 无 TTL 的僵尸 key，标记清理
        cleanupKeys.push(keys[index])
        statusMap.set(`${accountType}:${accountId}`, false)
      } else {
        statusMap.set(`${accountType}:${accountId}`, ttl > 0)
      }
    })
    ...
    if (cleanupKeys.length > 0) {
      const cleanupPipeline = client.pipeline()
      cleanupKeys.forEach((key) => cleanupPipeline.del(key))
      cleanupPipeline.exec().catch(() => {})
      logger.warn(
        `⚠️ [UpstreamError] Auto-cleared ${cleanupKeys.length} temp_unavailable key(s) without TTL`
      )
    }
