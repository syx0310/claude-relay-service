...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    const clientLabel = isCodexCLI
      ? '✅ Codex CLI request detected'
      : '📝 Non-Codex CLI request detected'

    const defaultCodexCliInstructions =
    ...
    const adapterResult = adaptCodexRequestBody(req.body, {
      isCodexCLI,
      adapterConfig: config?.openai?.codexAdapter,
      defaultInstructionsText: defaultCodexCliInstructions
    })
    ...
    if (adapterResult.applied) {
      ...
      logger.info(
        clientLabel +
          ', codexAdapter: instructions=' +
          instructionsMode +
          ', stripped_fields=' +
          strippedCount
      )
      req.body = adapterResult.body
    } else {
      logger.info(clientLabel + ', forwarding as-is')
    }
