...
// 提取 axios/普通错误的关键信息，避免将整个 error 对象（含数十 MB 请求体）传入 logger
  ...
  if (error?.isAxiosError) {
    ...
    if (resData !== null && resData !== undefined) {
      if (typeof resData === 'string') {
        meta.responseData = resData.substring(0, 500)
      } else if (resData.error?.message) {
        meta.responseData = String(resData.error.message).substring(0, 500)
      } else if (typeof resData === 'object') {
        try {
          meta.responseData = JSON.stringify(resData).substring(0, 500)
        } catch (_) {
          meta.responseData = '[Unserializable response data]'
        }
      }
    }
    return meta
...
async function getOpenAIAuthToken(apiKeyData, sessionId = null, requestedModel = null) {
  try {
    ...
    logger.error('Failed to get OpenAI auth token:', safeErrorMeta(error))
    throw error
  }
}
...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    // 429 静默切号：选账户 → 发请求 → 429? 标记限流 → 重新选账户 → 重试
    const MAX_HTTP_SWITCH_RETRIES = 3
    ...
    for (let switchAttempt = 0; ; switchAttempt++) {
      ...
      // 如果是 OpenAI-Responses 账户，使用专门的中继服务处理（内部自行处理 429 切号）
      ...
      if (upstream.status === 429) {
        ...
        // 尝试静默切号重试
        if (switchAttempt < MAX_HTTP_SWITCH_RETRIES) {
          logger.info(
            `🔄 [HTTP] 429 on account ${accountId}, silent switch ${switchAttempt + 1}/${MAX_HTTP_SWITCH_RETRIES}`
          )
    ...
    if (!isStream) {
      ...
      try {
        ...
        logger.error('Failed to process non-stream response:', safeErrorMeta(error))
        if (!res.headersSent) {
    ...
    upstream.data.on('error', (err) => {
      logger.error('Upstream stream error:', safeErrorMeta(err))
      if (!res.headersSent) {
    ...
    logger.error('Proxy to ChatGPT codex/responses failed:', safeErrorMeta(error))
