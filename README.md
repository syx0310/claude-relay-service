...
本仓库 fork 自 [Wei-Shaw/claude-relay-service](https://github.com/Wei-Shaw/claude-relay-service)，定期同步上游更新。以下为本 fork 在上游基础上新增/修改的功能：

### Codex WebSocket 模式中继

- 支持 Codex CLI 的 WebSocket 长连接模式（`/openai/responses` WS 路径）
- 懒连接：首条客户端消息时才建立上游 WebSocket
- 429/401 时自动断开上游、保持客户端连接，下次消息重选账户
- 空闲超时（默认 5 分钟）和最大连接生命周期（默认 60 分钟）管理
- 解析 `response.completed` 事件追踪 Usage

### 429 静默账户切换

- **HTTP 路径**：OpenAI Responses 请求遇上游 429 时，自动切换账户重试（最多 3 次），客户端无感知
- **WebSocket 路径**：拦截 429 error event，不转发给客户端，静默切号后重发请求

### Anthropic ↔ Codex 格式桥接

- 在 Claude API 端点（`/api/`）支持 Codex 模型请求
- 自动将 Anthropic Messages API 格式转换为 OpenAI Responses API 格式，双向 SSE 流转换
- 工具调用格式自动适配

### Codex 请求适配器

- 对非 Codex CLI 客户端的请求自动注入/覆盖 `instructions`，可配置三种模式（`overwrite` / `prepend` / `none`）
- 支持环境变量和 `config/config.js` 配置

### 性能优化

- **日志安全**：防止序列化巨大 error 对象导致 CPU/内存峰值（深度和大小限制）
- **请求日志精简**：非 debug 级别仅记录请求摘要（model、消息数等），不再序列化完整请求/响应体，显著降低 CPU 和内存开销
- **调度器 Redis 优化**：新增 `getAllAccountsForScheduler()` 轻量加载，跳过 UI 格式化和冗余查询；限流判断改为内存计算（`_isAccountRateLimitedFromData`），减少调度路径上的 Redis 往返
